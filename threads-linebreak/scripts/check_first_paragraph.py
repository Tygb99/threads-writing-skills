...
EVENT_RE = re.compile(
    ...
    # 내가 한 일이 아니라 나에게 닥친 고장·사고
    r"|뻗었|멈췄|죽었|먹통|고장|날아갔|잠겼|튕겼|안 먹|못 쓰|날렸)"
)

# 내가 만들거나 고친 것 — 주어가 작업물이라는 신호
ARTIFACT_RE = re.compile(
    # '업데이트'처럼 남이 한 일에도 쓰이는 낱말은 **내가 한 동사꼴일 때만** 작업물로 본다.
    # "코덱스 업데이트 한번에 맥이 뻗었다"는 사건이지 내 작업물이 아니다(2026-08-14 오판).
    r"(만들었|공개합니다|공개한다|정리했|올립니다|지웠|깎|붙였|바꿨|추가했|업데이트했|업데이트하|고쳤|출시했|배포했|런칭|완성했)"
)
